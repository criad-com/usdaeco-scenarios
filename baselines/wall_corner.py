"""Metre L+T wall baseline with a cut opening and a door filling; adapted from Wall fixtures."""
import ifcopenshell
...
def build(output):
    ...
    for i in range(3):
        ...
        matrix=np.eye(4)
        if i:
            matrix[:3,0]=(0,1,0); matrix[:3,1]=(-1,0,0); matrix[:3,3]=(4 if i == 1 else 2,0,0)
    ...
    run('geometry.connect_path',model,relating_element=walls[0],related_element=walls[2],relating_connection='ATPATH',related_connection='ATSTART')
    for wall in walls:
        run('geometry.regenerate_wall_representation', model, wall=wall)
    ...
    rep=run('geometry.add_wall_representation',model,context=body,length=.9,height=2.1,thickness=.04)
